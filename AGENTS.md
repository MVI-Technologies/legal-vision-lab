...
### Core Product Engineering Lessons (Refined in Luxury Barber & Martins S.A. Redesigns)
...
- **Mobile Widget Isolation (React Portals for Floating Elements)**: Fixed interactive widgets (such as floating chat widgets or FABs) must be isolated from parent CSS flex/grid containers which often create stacking contexts trapping the element on specific mobile viewports (e.g., 412x915). Use `createPortal` to render these floating layers directly into `document.body` with inline styles (`style={{ zIndex: 2147483647 }}`).
- **Zero-Dependency Styling for Global Components**: When introducing dynamic or deferred components in client-side apps, do not risk layout breaking due to delayed Tailwind compilation. Key universal layouts (like floating widgets) should rely on raw React inline styles with hardcoded color values to ensure 100% visual stability across all preview viewports.
- **Prestige Spacing Density**: Modern high-end boutique landing pages feel more integrated and dense when moving away from generic open spacing (`py-24`/`py-32`) to a more publication-like, tighter spacing structure (`py-16` / `pt-16 pb-12` for headings) across every single page.
- **Typography Posture ("No Italics" Rule)**: In institutional niches demanding ultimate authority and seriousness, design character is preserved by completely avoiding italicized fonts. Relying exclusively on straight, solid serif headlines (e.g., Playfair Display) paired with straight body text ensures maximum stature.
